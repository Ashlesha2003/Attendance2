...
def mark_attendance(name):
    ...
    # Check both roll number and name to avoid duplicates
    already_marked = ((df["Roll Number"] == roll_number) & (df["Name"] == student_name)).any()
    ...
    if not already_marked:
        ...
        st.warning(f"Attendance already marked for {name}")
...
def take_attendance_page():
    ...
    if uploaded_file is not None:
        ...
        if recognized_students:
            ...
            st.success(f"Attendance complete: {recognized_count} recognized, {unrecognized_count} unrecognized.")
        else:
            if unrecognized_count > 0:
                st.warning(f"{unrecognized_count} face(s) detected but not confident enough to recognize.")
            else:
